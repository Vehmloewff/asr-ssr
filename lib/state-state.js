// From https://github.com/TehShrike/abstract-state-router/blob/master/lib/state-state.js
...
module.exports = function StateState() {
	...
	function guaranteeAllStatesExist(newStateName) {
		...
		if (statesThatDontExist.length > 0) {
			throw new Error(
				`State ${
					statesThatDontExist[statesThatDontExist.length - 1]
				} does not exist`
			);
		}
	}
